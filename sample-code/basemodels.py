...
class MeetingSummary(BaseContentSummary):
    ...
    content_type: Literal["meeting"] = Field(
        default="meeting", description="Must be exactly 'meeting' for meeting summaries"
    )
...
class TechnicalArticleSummary(BaseContentSummary):
    ...
    source_url: str | None = Field(
        None,
        description="Link to the original article. DONT MAKE IT UP IF YOU CAN'T FIND IT.",
    )
    publication_date: str | None = Field(
        None,
        description="Date the article was published, if known. DONT MAKE IT UP IF YOU CAN'T FIND IT.",
    )
    publisher: str | None = Field(
        None,
        description="Name of the publisher or platform. DONT MAKE IT UP IF YOU CAN'T FIND IT.",
    )
    ...
    llm_ready_metadata: str | None = Field(
        None,
...
class NewsArticleSummary(BaseContentSummary):
    ...
    author: str | None = Field(
        None,
        description="Author of the article, if mentioned. DONT MAKE IT UP IF YOU CAN'T FIND IT.",
    )
...
class ContextHint(BaseModel):
    ...
    hint_data: list[str] | list[dict[str, str]] | dict[str, str] | str = Field(
        ..., description="The actual context data - can be list, dict, or string"
    )
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Confidence in this hint's accuracy"
    )
    source: str = Field(
        default="agent",
...
class BaseContentState(TypedDict):
    ...
    summary_model: (
        BaseModel | None
    )  # Will be populated with MeetingSummary, NewsSummary, etc.
    structured_output: dict[str, Any]
    final_output: dict[str, Any]
    context_hints: ContextHints | None  # New field
